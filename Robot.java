...
public class Robot extends SampleRobot {
	...
	// new button config
	final int FIRE_BTN = 5, FLYIN_BTN = 2, FLYOUT_BTN = 1, WHEELIE_BTN = 3, RLEFT_BTN = 7, RRIGHT_BTN = 8;
	...
	public void robotInit() {
		...
		fly1.changeControlMode(CANTalon.ControlMode.Speed);
		fly2.changeControlMode(CANTalon.ControlMode.Speed);
		fly1.set(0);
		fly2.set(0);
		...
		while (isOperatorControl() && isEnabled()) {
			...
			if (Math.abs(x) > DEADZONEX || Math.abs(y) > DEADZONEY) {
				...
				if (!drivestick.getRawButton(WHEELIE_BTN)) {
					...
					bogieLeft2.set(trimLeft * (x + -y)); // these two need to be
															// reversed
